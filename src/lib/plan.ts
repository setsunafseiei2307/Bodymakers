...
/** 速さの判定。推奨帯を基準に4段階で返す。 */
export type PaceVerdict = 'gentle' | 'recommended' | 'fast' | 'unrealistic';
...
/** 推奨帯の上限の何倍までを「速い」とみなすか。これを超えると「無理がある」。 */
const UNREALISTIC_FACTOR = 1.5;
...
export function judgePace(weeklyPercent: number, mode: PlanMode): PaceVerdict {
  ...
  if (weeklyPercent <= band.max * UNREALISTIC_FACTOR) return 'fast';
  return 'unrealistic';
}
